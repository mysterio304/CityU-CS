...
$\text{VALID\\_EMP\\_DEP} \leftarrow \sigma_{\text{Dependent.Essn = Employee.Ssn}}(\text{EMP\\_DEP})$

$\text{RESULT} \leftarrow \rho_{\text{Dependent\_name, Fname, Lname}}(\pi_{\cdots}(\text{VALID\\_EMP\\_DEP}))$
